 ...
 * or immediately after the key long, meaning that the data can be inserted
 * after a space char
 *
 * 	keys can be merged together if they are flags (i.e: tar -xzvf ...) or,
 * 	if ARG_PARSE_MERGED is passed to the arg_parse function, merged
 * key-value arguments can be used (i.e objdump -Mintel ...) */
...
typedef arg_return p_arg_handler (char *data_ptr, size_t blksize,
                                  void *retval);
 ...
 *   const char * long_arg: a long variant of the argument's key (i.e:
 * --recursive) arg_handler handler: a handler for the value to be parsed
...
ARGPARSE_API char *arg_parse (int *argc, char ***argv, arg_list list,
                              char **not_keys, size_t *not_keys_size,
                              arg_flags flags, arg_return *return_code);
