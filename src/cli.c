...
static ARG_INLINE size_t
arg_strlen (const char *str)
{
  size_t len = 0;
  while (*str)
    {
...
static ARG_INLINE char *
arg_strcpy (char *dest, const char *src)
{
  while (*src)
    {
      *dest++ = *src++;
...
static ARG_INLINE void *
arg_memcpy (void *dest, const void *src, size_t n)
{
  while (*(char *)dest)
    {
      *(char *)dest++ = *(char *)src++;
...
static ARG_INLINE int
arg_strcmp (const char *f, const char *s)
{
  while (*f && *s)
    {
      if (!(*f++ == *s++))
...
arg_return
arg_string_handler (char *data_ptr, size_t blksize, void *retval)
{
...
arg_return
arg_strcpy_handler (char *data_ptr, size_t blksize, void *retval)
{
  if (blksize == 0)
    {
      retval = NULL;
...
static ARG_INLINE void
arg_parse_value (struct arg_state *state, void **data, size_t *size)
{
...
static ARG_INLINE arg_return
arg_call_handler (struct arg_state *state)
{
  if (state->ptr->flags & ARG_FLAG_OPTIONAL)
    {
      if ((state->type == ARG_SHORT && *(*state->argv + 1) != 0x0)
  ...
  if (state->ptr->handler)
    {
      ...
      if ((code = state->ptr->handler (data, size, state->ptr->retval))
          != ARG_SUCCESS)
        {
          return code;
        }
...
static ARG_INLINE arg_return
arg_parse_short (struct arg_state *state)
{
  size_t i;
  char found;
...
static ARG_INLINE arg_return
arg_parse_long (struct arg_state *state)
{
  size_t i;
  arg_return code;
  for (i = 0; i < state->len; ++i)
...
char *
arg_parse (int *argc, char ***argv, arg_list list, char **nk_buf,
           size_t *nk_size, arg_flags flags, arg_return *code)
{
