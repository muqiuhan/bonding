...
// Used for printing from within the logger. Prints if debug level is
// SIMPLOG_DEBUG or higher
#define SIMPLOG_LOGGER 4
// Used for printing stack trace. Prints if debug level is SIMPLOG_DEBUG or
// higher
...
/*
   ...
   immediately -1 : Error          - An error has occured: program may not exit
   ...
   levels. const char* str - The message to be output. This is a format string.
...
writeLog (int loglvl, const char *str, ...)
{
  ...
  int va_string_size
      = vsnprintf (va_msg, strlen (str) + max_va_list_size, str, args);
  ...
  int msgSize = stringSize + lineWrapSize + strlen (strerror (errno))
                + 50; // 50 char buffer to prevent overflow
  ...
  if (loglvl < SIMPLOG_INFO)
    ...
    {
      // Used to check if a valid combination of log level and debug level
      // exists
      bool valid = true;
  ...
  if (va_string_size > (strlen (str) + max_va_list_size))
    {
      ...
      writeLog (SIMPLOG_LOGGER,
                "Previous message truncated by %d bytes to fit into buffer",
                truncated_size);
    }
}
...
writeStackTrace ()
{
  ...
  char **backtrace_strings
      = getPrettyBacktrace (backtrace_addresses, backtrace_size);
  if (backtrace_strings == NULL)
    {
      backtrace_strings
          = backtrace_symbols (backtrace_addresses, backtrace_size);
      freePrettyBacktrace = false;
    }
  // Clear errno
  // It is possible errno is set to a value we don't care about by
  // 'backtrace_symbols'
  errno = 0;

  // max size for the message, assuming individual strings with max of 255
  // bytes
      ...
      if (i < (backtrace_size - 1))
        {
          ...
          message[offset] = '\n';
          offset += 1;
          strncpy (message + offset, indentedLineSpacing,
                   strlen (indentedLineSpacing));
...
setLogDebugLevel (int level)
{
  ...
  if (level >= SIMPLOG_INFO && level <= SIMPLOG_VERBOSE)
  ...
  else
    {
      ...
      sprintf (
          error,
          "Invalid debug level of '%d'. Setting to default value of '%d'\n",
          level, SIMPLOG_DEBUG);
      ...
      sprintf (error + strlen (error), "%s3  : Debug-Verbose",
               indentedLineSpacing);
...
/*
    ...
    Input:
    bool wrap - Desired state of line wrapping: true = Enabled (default), false
   = Disabled
*/
void
      ...
      if (!silentMode)
        {
          ...
          printf ("%s\tLOG   : Logfile '%s' does not exist. It will be "
                  "created now.\n",
...
loadConfig (const char *config)
{
  ...
  while ((bytesRead = read (fd + bytesRead, settingsBuffer, SETTINGS_BUFSIZE))
         > 0)
    {
      ...
      if (bytesRead == SETTINGS_BUFSIZE)
        {
          settingsBuffer
              = (char *)realloc ((void *)settingsBuffer, SETTINGS_BUFSIZE * 2);
        }
    }
...
getPrettyBacktrace (void *addresses[], int array_size)
{
  ...
  char command_string[max_command_string_size]; // set to the maximum possible
                                                // path size
  ...
  if (!system ("which gaddr2line > /dev/null 2>&1"))
    {
      ...
      if (path_length <= 0)
        {
          writeLog (SIMPLOG_LOGGER, "Unable to get execution path. Defaulting "
                                    "to standard backtrace.");
          error = true;
        }
    ...
    {
      writeLog (SIMPLOG_LOGGER,
                "Function 'gaddr2line' unavailable. Defaulting to standard "
                "backtrace. Please install package 'binutils' for better "
                "stacktrace output.");
      error = true;
    }
#else
  ...
  if (!system ("which addr2line > /dev/null 2>&1"))
    ...
    {
      writeLog (SIMPLOG_LOGGER,
                "Function 'addr2line' unavailable. Defaulting to standard "
                "backtrace. Please install package 'binutils' for better "
                "stacktrace output.");
      error = true;
    }
  ...
  for (int i = 0; i < array_size; i++)
    {
      ...
      sprintf (command_string, "%s \"%s\" %X 2>/dev/null", command, exe_path,
      ...
      if (line == NULL)
        {
          writeLog (SIMPLOG_LOGGER,
                    "Failed to execute command: '%s'. Defaulting to standard "
                    "backtrace.",
                    command);
          for (int i = 0; i < array_size; i++)
            {
      ...
      if (fgets (backtrace_strings[i], 255, line) == NULL)
        {
          writeLog (SIMPLOG_LOGGER, "Failed to get pretty backtrace strings. "
                                    "Defaulting to standard backtrace.");
          for (int i = 0; i < array_size; i++)
            {
              free (backtrace_strings[i]);
            }
      ...
      if (strcmp (backtrace_strings[i], "??") != 0
          && strcmp (backtrace_strings[i], "?? ??:0") != 0)
        {
  ...
  // If no addresses were evaluated successfully, we fall back on the standard
  // backtrace
  if (!address_evaluation_successful)
    {
      writeLog (SIMPLOG_LOGGER,
                "Command '%s' failed to evaluate addresses. Defaulting to "
                "standard backtrace.",
                command);
      for (int i = 0; i < array_size; i++)
        {
      ...
      if (strlen (msg + i) + strlen (indentedLineSpacing) > 80)
        {
          ...
          lineFeedSize = (strlen (msg + i) < 80 ? strlen (msg + i) : 80)
                         - strlen (indentedLineSpacing) - strlen (wrapText);
...
simplog_namespace const simplog
    = { writeLog,         writeStackTrace, setLogDebugLevel, setLogFile,
        setLogSilentMode, setLineWrap,     flushLog,         loadConfig };