...
TEST_CASE("structopt can parse fixed array positional argument"
          * test_suite("array_positional"))
{
  // Array of ints
  {
  ...
  }
  {
    ...
    REQUIRE(arguments.value
            == std::array<float, 5>{ 0.1e1f, -0.2e2f, +0.3e3f, -0.4e4f, +5e5f });
  }
  {
    auto arguments = structopt::app("test").parse<ArrayFloatArgument>(
  ...
  // Array of bools
  {
    auto arguments = structopt::app("test").parse<ArrayBoolArgument>(
      std::vector<
        std::string>{ "./main", "true", "false", "on", "off", "yes", "no", "1", "0" });
    REQUIRE(arguments.value
