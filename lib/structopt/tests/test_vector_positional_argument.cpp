...
TEST_CASE("structopt can parse vector positional argument"
...
TEST_CASE("structopt can parse vector of pairs positional argument"
          * test_suite("vector_positional"))
{
  {
    auto arguments =
  ...
  }
  {
    ...
    REQUIRE(arguments.values
            == std::vector<std::pair<std::string, int>>{ { "a", 1 }, { "b", 2 } });
  }
  {
    auto arguments = structopt::app("test").parse<VectorOfPairs>(
