...
namespace doctest
{
  ...
  || defined(DOCTEST_CONFIG_INCLUDE_TYPE_TRAITS)
    ...
    template <bool C> struct StringMakerBase
    {
      template <typename T>
      static String
      convert(const DOCTEST_REF_WRAP(T))
      {
        return "{?}";
      }
    };
    ...
    template <> struct StringMakerBase<true>
    {
      template <typename T>
      static String
      convert(const DOCTEST_REF_WRAP(T) in)
      {
        *getTlsOss() << in;
...
#define DOCTEST_BINARY_RELATIONAL_OP(n, op) \
    ...
    struct DOCTEST_INTERFACE ResultBuilder : public AssertData
    {
      ...
      template <typename L>
      DOCTEST_NOINLINE void
      unary_assert(const DOCTEST_REF_WRAP(L) val)
      {
        m_failed = !val;
  ...
  throwException()
      ...
      : public IExceptionTranslator //! OCLINT destructor of virtual class
    {
     public:
      ...
      bool
      translate(String & res) const override
      {
#ifndef DOCTEST_CONFIG_NO_EXCEPTIONS
        try
          {
    ...
    template <bool C> struct StringStreamBase
    {
      template <typename T>
      static void
      convert(std::ostream * s, const T & in)
      {
        *s << toString(in);
      }
      ...
      static void
      convert(std::ostream * s, const char * in)
      {
        *s << String(in);
      }
    };
    ...
    template <typename L> class DOCTEST_INTERFACE ContextScope : public ContextScopeBase
    {
     ...
     public:
      ...
      void
      stringify(std::ostream * s) const override
      {
        lambda_(s);
      }
...
#define DOCTEST_DEFINE_DECORATOR(name, type, def)                                        \
  ...
  {                                                                                      \
    ...
    void                                                                                 \
    fill(detail::TestCase & state) const                                                 \
    {                                                                                    \
      state.DOCTEST_CAT(m_, name) = data;                                                \
    }                                                                                    \
    void                                                                                 \
    fill(detail::TestSuite & state) const                                                \
    {                                                                                    \
      state.DOCTEST_CAT(m_, name) = data;                                                \
    }                                                                                    \
  }
...
#define DOCTEST_TEST_CASE_TEMPLATE_DEFINE(dec, T, id)                                    \
  DOCTEST_TEST_CASE_TEMPLATE_DEFINE_IMPL(dec,                                            \
                                         T,                                              \
...
#define DOCTEST_TEST_CASE_TEMPLATE_INVOKE(id, ...)                                       \
  DOCTEST_TEST_CASE_TEMPLATE_INSTANTIATE_IMPL(id,                                        \
                                              DOCTEST_ANONYMOUS(_DOCTEST_ANON_TMP_),     \
...
#define DOCTEST_TEST_CASE_TEMPLATE(dec, T, ...)                                          \
  DOCTEST_TEST_CASE_TEMPLATE_IMPL(dec,                                                   \
                                  T,                                                     \
                                  DOCTEST_ANONYMOUS(_DOCTEST_ANON_TMP_),                 \
                                  __VA_ARGS__)
...
#define DOCTEST_REGISTER_EXCEPTION_TRANSLATOR(signature)                                 \
  DOCTEST_REGISTER_EXCEPTION_TRANSLATOR_IMPL(                                            \
    DOCTEST_ANONYMOUS(_DOCTEST_ANON_TRANSLATOR_),                                        \
    signature)
...
#define DOCTEST_INFO_IMPL(lambda_name, mb_name, s_name, expression)                      \
  ...
  auto lambda_name = [&](std::ostream * s_name) {                                        \
    doctest::detail::MessageBuilder mb_name(__FILE__,                                    \
                                            __LINE__,                                    \
...
// clang-format on
  ...
  namespace
  {
    ...
    std::ostream &
    file_line_to_stream(std::ostream & s,
                        const char * file,
                        int line,
                        const char * tail = "")
    {
...
// =================================================================================================
    ...
    struct XmlReporter : public IReporter
    {
      ...
      unsigned
      line(unsigned l) const
      {
        return opt.no_line_numbers ? 0 : l;
      }

      void
      test_case_start_impl(const TestCaseData & in)
      {
      ...
      void
      report_query(const QueryData & in) override
      {
        test_run_start();
      ...
      void
      test_run_end(const TestRunStats & p) override
      {
      ...
      void
      test_case_start(const TestCaseData & in) override
      {
      ...
      void
      test_case_reenter(const TestCaseData &) override
      {
      }

      void
      test_case_end(const CurrentTestCaseStats & st) override
      {
      ...
      void
      test_case_exception(const TestCaseException & e) override
      {
      ...
      void
      subcase_start(const SubcaseSignature & in) override
      {
        std::lock_guard<std::mutex> lock(mutex);
      ...
      void
      subcase_end() override
      {
        xml.endElement();
      }

      void
      log_assert(const AssertData & rb) override
      {
      ...
      void
      log_message(const MessageData & mb) override
      {
        std::lock_guard<std::mutex> lock(mutex);
      ...
      void
      test_case_skipped(const TestCaseData & in) override
      {
        if (opt.no_skipped_summary == false)
          {
            test_case_start_impl(in);
    ...
    struct ConsoleReporter : public IReporter
    {
      ...
      Color::Enum
      getSuccessOrFailColor(bool success, assertType::Enum at)
      {
      ...
      void
      successOrFailColoredStringToStream(bool success,
                                         assertType::Enum at,
      ...
      void
      subcase_start(const SubcaseSignature & subc) override
      {
        std::lock_guard<std::mutex> lock(mutex);
  ...
  {                                                                                      \
                   ...
                   int & res)
    {
      ...
      if (type == 0)
        {
          // boolean
          const char positive[][5] = { "1",
                                       "true",
                                       "on",
                                       "yes" }; // 5 - strlen("true") + 1
          const char negative[][6] = { "0",
                                       "false",
                                       "off",
                                       "no" }; // 6 - strlen("false") + 1
